...
| ASS 已验收；双字幕已实现，待实机验收 | `E4-LIBASS` | Exo/字幕 | ASS 特效字幕及对齐 MPV 默认行为的主／副字幕 | 2026-09-18 完成单 ExoPlayer 双 TextRenderer、独立选轨/关闭、置顶副显示层及媒体代次隔离；13 项本机合约/真实播放器/View 接线检查通过，手机 Release/Debug/测试 APK 与电视 32 位 Java 编译通过；手机未连接，原生同屏尚未实机验收。此前外挂/容器兼容桥和原字体已验收，ASS 常规构建必需；复杂字幕已接受一核 CPU ≤40% / render+upload p95 ≤16.67 ms。主 ASS、MPV 独立修复保留；精确 MKV duration/未缓存长事件 seek、libass 双 ABI、HDR/DV 等历史边界见第 14–16 节 | [E4-LIBASS-exo-ass-rendering.md](E4-LIBASS-exo-ass-rendering.md) |
